...
# 3 "BOARD1_Guncel.asm" 2

    ; --- DONANIM YAPILANDIRMASI ---
    ; Osilator: HS, WDT: Kapali, PWRT: Acik, Koruma: Kapali
    CONFIG FOSC = HS, WDTE = OFF, PWRTE = ON, BOREN = OFF, LVP = OFF, CPD = OFF, WRT = OFF, CP = OFF

;------------------------------------------------------------------------------
; BELLEK HARITASI VE DEGISKENLER (Makyajli Isimler)
;------------------------------------------------------------------------------
    PSECT udata_bank0

    ; --- 7-Segment Surucu Verileri ---
    SEG_1: DS 1 ; Yuzler/Onlar basamagi
    SEG_2: DS 1 ; Onlar/Birler basamagi
    SEG_3: DS 1 ; Birler/Ondalik basamagi
    SEG_4: DS 1 ; Sembol (C veya F)
    MUX_INDEX: DS 1 ; Tarama sirasi sayaci

    ; --- Zamanlayici ve Durumlar ---
    SEC_TICK: DS 1 ; 1 Saniye sayaci
    VIEW_STATE: DS 1 ; 0:Oda, 1:Ayar, 2:Fan
    VIEW_TMR: DS 1 ; Ekran degisim zamanlayicisi

    ; --- Giris Birimi (Keypad) ---
    BTN_CODE: DS 1 ; Okunan tus degeri
    BTN_STAT: DS 1 ; Tus basili mi? (Flag)
    IS_TYPING: DS 1 ; Veri giris modu aktif mi?
    TYPE_STEP: DS 1 ; Giris adimi (Onlar->Birler...)

    ; --- Fiziksel Degerler ---
    SET_TEMP: DS 1 ; Hedef Sicaklik (2x skalali)
    ROOM_TEMP: DS 1 ; Ortam Sicakligi (LM35)
    FAN_SPEED: DS 1 ; Fan Devri (RPS)

    ; --- Gecici Bellekler ---
    VAL_INT: DS 1 ; Tam sayi kismi (Giris)
    VAL_FRAC: DS 1 ; Ondalik kisim (Giris)
    RX_BUF: DS 1 ; UART Alim Tamponu
    PC_CMD_INT: DS 1 ; PC'den gelen tam sayi
    PC_CMD_FRAC: DS 1 ; PC'den gelen ondalik

    ; --- Matematik ve Dongu ---
    MATH_VAR: DS 1 ; Hesaplama degiskeni
    TX_BUF: DS 1 ; Gonderim tamponu
    WAIT_A: DS 1 ; Gecikme sayaci 1 (Eski D1)
    WAIT_B: DS 1 ; Gecikme sayaci 2 (Eski D2)
    DEBOUNCE_T: DS 1 ; Ark onleme sayaci (Eski SCAN_D)
    LUT_PTR: DS 1 ; Tablo isaretcisi (Eski SEG_TEMP)

    ; --- Kesme Saklama Alanlari ---
    PSECT udata_shr
    W_SAVE: DS 1 ; W Register Yedegi
    ST_SAVE: DS 1 ; STATUS Register Yedegi
...
;==============================================================================
    ...
    ; 1. Seri Haberlesme (UART) Kontrolu
    BANKSEL PIR1
    BTFSC PIR1, 5 ; ((PIR1) and 07Fh), 5 Set mi?
    CALL ISR_UART_TASK

    ; 2. Ekran Tarama (Timer1)
    BANKSEL PIR1
    BTFSS PIR1, 0 ; ((PIR1) and 07Fh), 0 Set mi?
    GOTO EXIT_ISR

    BCF PIR1, 0 ; Bayragi Temizle

    ; Timer1 Yukle (4ms tarama hizi)
    MOVLW 0xF0
    MOVWF TMR1H
    MOVLW 0x00
    MOVWF TMR1L

    CALL UPDATE_7SEG ; Bir sonraki haneyi yak
...
ONE_SEC_TASK:
    ...
    ; Ekran Modunu Degistir (Oda -> Hedef -> Fan)
    INCF VIEW_TMR, F
    BTFSS VIEW_TMR, 1 ; 2. saniyede degis
    GOTO EXIT_ISR
    CLRF VIEW_TMR
    INCF VIEW_STATE, F
    MOVF VIEW_STATE, W
    SUBLW 3
    BTFSC STATUS, 2 ; 3 olduysa sifirla
    CLRF VIEW_STATE
...
ISR_UART_TASK:
    ...
    ; SET Komutlari (Maskeleme ile kontrol)
    MOVF RX_BUF, W
    ANDLW 0xC0
    XORLW 0xC0 ; 11xxxxxx formati?
    BTFSC STATUS, 2
    GOTO RX_SET_TARGET

    GOTO ISR_UART_END

; --- Gonderim Fonksiyonlari ---
TX_TARGET_FRAC:
    MOVLW 0
    BTFSC SET_TEMP, 0 ; LSB kontrol (0.5 var mi?)
    MOVLW 5
    CALL UART_SEND
    GOTO ISR_UART_END
...
APPLY_PC_TARGET:
    ...
    MOVWF SET_TEMP
    BCF STATUS, 0
    RLF SET_TEMP, F ; 2 ile carp (Skala)
    GOTO ISR_UART_END
...
UART_SEND:
    ...
    BANKSEL TXSTA
WAIT_TX:
    BTFSS TXSTA, 1 ; ((TXSTA) and 07Fh), 1 bos mu?
    GOTO WAIT_TX
    BANKSEL TXREG
    MOVF TX_BUF, W
    MOVWF TXREG
...
INIT_SYSTEM:
    ; Bank 1 Ayarlari
    BSF STATUS, 5
    CLRF TRISD ; PortD Cikis (Segment Data)
    CLRF TRISC ; PortC Cikis (Digit Select)
    MOVLW 0xF0 ; ((PORTB) and 07Fh), 4 -7 Giris (Keypad Col), ((PORTB) and 07Fh), 0 -3 Cikis
    MOVWF TRISB
    MOVLW 0x11 ; ((PORTA) and 07Fh), 0 Analog, ((PORTA) and 07Fh), 4 Input
    MOVWF TRISA

    ; UART Konfigurasyonu (9600 Baud)
    MOVLW 25
    MOVWF SPBRG
    BSF TXSTA, 2 ; ((TXSTA) and 07Fh), 2 Yuksek Hiz
    BCF TXSTA, 4 ; Asenkron
    BSF TXSTA, 5 ; TX Aktif

    ; Kesme Yetkileri
    BSF PIE1, 0 ; Timer1 Kesmesi
    BSF PIE1, 5 ; RX Kesmesi

    ; Analog Ayarlar
    MOVLW 0x8E ; AN0 Analog, Saga Dayali
    MOVWF ADCON1
    MOVLW 0x28 ; Timer0 Ayari
    MOVWF OPTION_REG

    ; Bank 0 Ayarlari
    BCF STATUS, 5
    BSF RCSTA, 7 ; ((RCSTA) and 07Fh), 7 Acik
    BSF RCSTA, 4 ; ((RCSTA) and 07Fh), 4 Acik

    MOVLW 0x01 ; Timer1 Baslat
    MOVWF T1CON

    BSF INTCON, 6 ; ((INTCON) and 07Fh), 6
    BSF INTCON, 7 ; ((INTCON) and 07Fh), 7 (Global Interrupt)

    MOVLW 0x81 ; ADC Modulu Acik
    MOVWF ADCON0
    ...
    ; Keypad Hazirligi (Satirlar High)
    BSF PORTA, 1
    BSF PORTA, 2
    BSF PORTA, 3
    BSF PORTA, 5
...
OP_LOOP:
    ...
    CALL READ_MATRIX
    MOVF BTN_STAT, F
    BTFSS STATUS, 2 ; Tus basili mi?
    CALL HANDLE_INPUT

    ; 2. Mod Kontrolu (Veri girisi varsa sistemi duraklatma)
    BTFSC IS_TYPING, 0
    GOTO PAUSE_SYS

    ; 3. Sistem Yonetimi
    CALL ADC_GET_VAL ; Sicaklik Oku
    CALL PREP_UI_DATA ; Ekrani Hazirla
    CALL MANAGE_HVAC ; Klima/Fan Kontrol

PAUSE_SYS:
    CALL WAIT_MS ; Gecikme
    CLRF BTN_STAT ; Tus bayragini temizle

    ; Debounce (Tus Birakma Beklemesi)
CHECK_RELEASE:
    MOVF PORTB, W
    ANDLW 0xF0
    XORLW 0xF0
...
WAIT_CONV:
    BTFSC ADCON0, 2 ; Bitmesini Bekle
    GOTO WAIT_CONV
    BANKSEL ADRESL
    MOVF ADRESL, W
    BANKSEL PORTB
    MOVWF ROOM_TEMP ; Degeri Kaydet
    RETURN

PREP_UI_DATA:
    ; Hangi moddaysak onu goster
    MOVF VIEW_STATE, W
    SUBLW 0
    BTFSC STATUS, 2
    GOTO UI_ROOM
...
MANAGE_HVAC:
    ...
    MOVF ROOM_TEMP, W
    SUBWF SET_TEMP, W
    BTFSC STATUS, 2 ; Esitse
    GOTO SYS_IDLE
    BTFSS STATUS, 0 ; Hedef < Ortam (Sogut)
    GOTO ACTIVATE_COOL
    GOTO ACTIVATE_HEAT
ACTIVATE_HEAT:
    BCF PORTB, 1
    BSF PORTB, 0 ; Isitici Aktif
    CLRF FAN_SPEED
    RETURN
ACTIVATE_COOL:
    BCF PORTB, 0
    BSF PORTB, 1 ; Sogutucu Aktif
    MOVLW 150
    MOVWF FAN_SPEED ; Fan Hizini Set Et
    RETURN
SYS_IDLE:
    BCF PORTB, 0
    BCF PORTB, 1
...
STEP_COMMIT:
    ...
    MOVWF SET_TEMP
    BCF STATUS, 0
    RLF SET_TEMP, F ; x2 (Yarim derece hassasiyeti)

    MOVF VAL_FRAC, W
    SUBLW 4 ; 5 ve uzeri mi?
    BTFSS STATUS, 0
    INCF SET_TEMP, F ; Evetse 0.5 ekle

    CLRF IS_TYPING ; Cikis
    RETURN

;==============================================================================
; MATRIX TARAMA (KEYPAD)
;==============================================================================
READ_MATRIX:
    CLRF BTN_CODE
    CLRF BTN_STAT
